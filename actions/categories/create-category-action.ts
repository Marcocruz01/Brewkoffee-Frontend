...
// Definimos el action
export async function createCategory(_prevState: any, formData: FormData): Promise<ActionStateType> {
    const categoryData = {
        name: formData.get('name'),
    }

    // Validamos contra el shape interno
    const validate = CreateCategorySchema.shape.body.safeParse(categoryData);
    ...
    try {
        ...
        const res = await fetch(`${process.env.API_BACKEND_URL}/categories`, {
            method: 'POST',
            headers: {
        ...
        if (!res.ok) {
            ...
            if (json.errors) {
                ...
                fieldErrors["general"] = [json.message || "Error creating category"];
            }
            return { errors: fieldErrors, success: '' }
        }
